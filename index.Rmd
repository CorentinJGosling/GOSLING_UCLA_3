...
# Questions pour toi Ariel

<b>Questions théoriques:</b><br>
- pour la validité convergente (on espère une corrélation forte), je pensais prendre GAD 7 et PHQ9 mais pas MBI car le MBI est manquant pour pas mal de personnes <br>
=> on prend aussi l'info sur le couple (ie, est-ce qu'on pense théoriquement que les personnes en couple se sentiront moins seules) ?<br>
- pour la validité discriminante (on espère une corrélation faible), je pensais prendre l'année d'étude et l'age (les difficultés financières auraient été bien mais il y a pas mal de données manquantes)<br>

<b>Questions pratiques</b><br>
- le sens est-il le bon pour l'UCLA => 1 faible lonelinne, 3/4 fort<br>
- il y a des items inversés sur le UCLA 20, tu peux m'indiquer lesquels?
- il y a des items inversés sur le MBI, tu peux m'indiquer lesquels?
- UCLA-3 est sur une échelle en 3pts et UCLA20 est en 4 pts, c'est correct? <br>
- le PHQ9 a 10 items, on est d'accord que j'enleve l'item 10 ?<br> 
- Si on garde le MBI, il y a le MBI-SS et MBI-HSS mais ils ne sont pas sur la même échelle, on peut faire qqch pour harmoniser les données sans passer par une standardisation des deux groupes ? 
- que fait-on des césures / interruption d'études si on garde cette variable pour la validité discriminante? pour l'année d'études ? On vire les données ? <br>
...
db_demo$Financ_diff[db_demo$Financ_diff=="Financ_diff"] <- "1"
...
dat = cbind(db_demo, db_uc3, db_uc20, db_phq, 
            db_gad, db_mbi_ss, db_mbi_hss)

# Financ_diff 
# UCLA3_TOT UCLA20_TOT PHQ_TOT GAD_TOT MBI_SS_TOT MBI_HSS_TOT
```
...
GGally::ggpairs(dat[,c("UCLA3_TOT", "UCLA20_TOT",
                       "PHQ_TOT","GAD_TOT",
...
heat = data.frame(apply(dat[,c(
               "PHQ_TOT","GAD_TOT","MBI_TOT",
               "Age_dim", "Financ_diff", "Year_stud", "UCLA3_TOT", "UCLA20_TOT")], 2, as.numeric))
...
approach 1: comparison of correlations (or lienar mixed models)
```{r}
cocor::cocor(~PHQ_TOT + UCLA3_TOT | PHQ_TOT + UCLA20_TOT,              
             data=dat)
```

approach 2: dominance analysis
...
approach 1: comparison of correlations (or lienar mixed models)
```{r}
cocor::cocor(~GAD_TOT + UCLA3_TOT | GAD_TOT + UCLA20_TOT,              
             data=dat)
```
